...
import org.openmobster.core.mobileCloud.rimos.module.sync.engine.SyncError;
	...
	throws SyncException
	{
		...
		if(!session.getSyncType().equals(SyncAdapter.SLOW_SYNC))
		{
			...
			for(int i=0,size=syncCommands.size();i<size;i++)
			{
				...
				VectorUtil.addAll(replyMessage.getStatus(), status);
			}
		}
		else
		{
		...
		if(!session.isOperationCommandStateInitiated())
		{
			try
			{
				if(!session.getSyncType().equals(SyncAdapter.SLOW_SYNC))
				{
					...
					VectorUtil.addAll(session.getAllOperationCommands(), addCommands);
					VectorUtil.addAll(session.getAllOperationCommands(), replaceCommands);
					VectorUtil.addAll(session.getAllOperationCommands(), deleteCommands);				
				}
				else
				{				
					...
					session.clearOperationCommandState();
					session.initiateOperationCommandState();
					VectorUtil.addAll(session.getAllOperationCommands(), slowSyncCommands);				
				}
			}
			catch(Exception e)
			{
