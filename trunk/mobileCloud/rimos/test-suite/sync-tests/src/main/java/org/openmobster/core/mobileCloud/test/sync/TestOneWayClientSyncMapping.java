 ...
 *
 */
public final class TestOneWayClientSyncMapping extends AbstractMapSyncTest
{		
	/**
	 * 
	 */
	public void runTest()
	{
		try
		{
			...
			this.assertRecordAbsence("unique-1-luid", this.getInfo()+"/add");
			this.assertRecordAbsence("unique-2-luid", this.getInfo()+"/add");
			...
			"<tag apos='apos' quote=\"quote\" ampersand='&'>1/Message</tag>", 
			...
			this.setUp("conflict");
			SyncService.getInstance().performOneWayClientSync(service, service, false);
			MobileObject conflictUnique1 = this.getRecord("unique-1-luid");
			MobileObject conflictUnique2 = this.getRecord("unique-2-luid");
