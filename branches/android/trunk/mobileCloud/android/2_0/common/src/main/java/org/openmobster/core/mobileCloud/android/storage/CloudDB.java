...
final class CloudDB
{
	...
	Set<String> listTables() throws DBException
	{
		...
		Cursor cursor = this.db.rawQuery("SELECT name FROM sqlite_master WHERE type=?", new String[]{"table"});
	...
	{
		Cursor cursor = this.db.rawQuery("SELECT name FROM sqlite_master WHERE type=? AND name=?", 
	...
	{
		Cursor cursor = this.db.rawQuery("SELECT count(*) FROM "+table, null);
	...
	Set<Record> selectAll(String from) throws DBException
	{
		...
		Cursor cursor = this.db.rawQuery("SELECT * FROM "+from, null);
	...
	{
		Record record = null;
		Cursor cursor = this.db.rawQuery("SELECT * FROM "+from+" WHERE recordid=?", new String[]{recordId});
