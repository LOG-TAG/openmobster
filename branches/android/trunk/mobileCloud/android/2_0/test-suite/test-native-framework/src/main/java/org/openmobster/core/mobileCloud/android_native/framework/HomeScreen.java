...
{	
	...
	public void postRender()
	{
		...
		//Find the run_button
		Button localError = (Button)ViewHelper.findViewById(currentActivity, 
		"localError");
		localError.setOnClickListener(
				new OnClickListener()
				{
					public void onClick(View clicked)
					{
						//Execute ShowErrorCommand
						CommandService service = Services.getInstance().getCommandService();
						CommandContext commandContext = new CommandContext();
						commandContext.setTarget("showerror");
						service.execute(commandContext);
					}
				}
		);
								
		//Find the remoteerror_button
		Button remoteError = (Button)ViewHelper.
		findViewById(currentActivity, 
		"remoteError");
		remoteError.setOnClickListener(new OnClickListener(){
			public void onClick(View clicked)
			{
				//Execute RemoteErrorCommand
				CommandService service = Services.getInstance().getCommandService();
				CommandContext commandContext = new CommandContext();
				commandContext.setTarget("remoteerror");
				service.execute(commandContext);
			}
		});
		
		//Find the remoteerror_button
		Button localScreen = (Button)ViewHelper.
		findViewById(currentActivity, 
		"localScreen");
		localScreen.setOnClickListener(new OnClickListener(){
			public void onClick(View clicked)
			{
