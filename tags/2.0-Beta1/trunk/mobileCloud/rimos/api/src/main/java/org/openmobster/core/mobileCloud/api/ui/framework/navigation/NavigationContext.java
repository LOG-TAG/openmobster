...
public final class NavigationContext 
{
	...
	private NavigationContext()
	{
		try
		{
			...
			while(screenIds.hasMoreElements())
			{
				...
				screenManager.register(screenId, (Screen)Class.forName(screenClassName).newInstance());
			}
		}
		catch(Exception e)
		{
	...
	{
		//Get the back screen		
		String backId = (String)this.history.pop();
	...
	{
		String currentId = (String)this.contextManager.getAttribute("current");
		ScreenManager.getInstance().forceRender(currentId);
	...
	{		
		...
		ScreenContext screenContext = (ScreenContext)AppStateManager.getInstance().getContext(screen);		
