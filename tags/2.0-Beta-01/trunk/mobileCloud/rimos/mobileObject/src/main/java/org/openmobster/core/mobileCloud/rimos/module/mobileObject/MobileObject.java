...
{	
	...
	private Vector arrayMetaData;
	
	
	public MobileObject()
	{
		this.fields = new Vector();
		this.arrayMetaData = new Vector();
	}
		
	MobileObject(Vector fields)
	{
		this.fields = fields;		
	}
	...
	{		
		...
		if(record.getValue("arrayMetaDataCount") != null)
		{
			...
			for(int index=0; index < count; index++)
			{
				...
				this.arrayMetaData.addElement(local);
			}
		}
	}
	...
	Record getRecord()
	{
		...
		if(this.fields != null && this.fields.size() > 0)
		{
			...
			Enumeration cour = this.fields.elements();
			int index = 0;
			while(cour.hasMoreElements())
			{
				Field field = (Field)cour.nextElement();				
		...
		if(this.arrayMetaData != null && this.arrayMetaData.size() > 0)
		{
			...
			Enumeration cour = this.arrayMetaData.elements();
			int index = 0;
			while(cour.hasMoreElements())
			{
				ArrayMetaData local = (ArrayMetaData)cour.nextElement();				
	...
	public Vector getArrayMetaData() 
	{
		return arrayMetaData;
	}

	public void setArrayMetaData(Vector arrayMetaData) 
	{
	...
	{	
		if(this.fields.isEmpty() || this.isCreatedOnDevice)
		{
			...
			{
				Field courField = (Field)this.fields.elementAt(i);
	...
	public void addToArray(String indexedPropertyName, Hashtable properties)
	{
		...
		{	
			...
			{
				Field local = (Field)this.fields.elementAt(indexOfLastElement);
				String localUri = local.getUri();
			...
			while(names.hasMoreElements())
			{
				...
				if(index != 0)
				{
					...
					{
						((Vector)this.fields).setElementAt(newField, insertionIndex);
					}
					else
					{
						((Vector)this.fields).addElement(newField);
					}										
				}
				else
				{
					...
					this.arrayMetaData.addElement(metaData);
				}
			}
	...
	public void removeArrayElement(String arrayUri, int elementAt)
	{
		...
		{
			Field local = (Field)this.fields.elementAt(i);
		...
		if(fieldsToDelete != null)
		{
			for(int i=0; i<fieldsToDelete.size(); i++)
			{
				...
				this.fields.removeElement(fieldToDelete);								
			}
	...
	public void clearArray(String indexedPropertyName)
	{
		...
		if(fieldsToDelete != null)
		{
			...
			{
				this.arrayMetaData.removeElement(arrayMetaData);
			}
		}
	}
	//-----------------------------------------------------------------------------------------------	
	private Field findField(String inputUri)
	{
		...
		if(this.fields != null)
		{
			for(int i=0,size=this.fields.size(); i<size; i++)
			{				
				boolean doesUriMatch = this.doesUriMatch(inputUri, ((Field)this.fields.elementAt(i)).getUri());
				if(doesUriMatch)
				{
	...
	private ArrayMetaData findArrayMetaData(String arrayUri)
	{
		...
		{	
			...
			{				
				ArrayMetaData metaData = (ArrayMetaData)this.arrayMetaData.elementAt(i);				
